...
if __name__ == "__main__":
    ...
    dt = 1.0e-10  # ok per v~1e6 m/s e k moderati
    steps = 13500 #1500 #1450
    ...
    g3 = qm._gradient_from_focal(phy.e, phy.mp, v03, L, f_target) * 0.8

    v04 = np.array([1e8, 0.0, 0.0])
    g4 = qm._gradient_from_focal(phy.e, phy.mp, v04, L, f_target) * 0.8
    ...
    QF2 = qm.Quadrupole(R, L, pos2, n_hat, g2, roll1, polarity=+1)  # F in un piano, D nell'altro
    QD2 = qm.Quadrupole(R, L, pos2 + np.array([L + S, 0, 0]), n_hat, g2, roll2, polarity=+1)  # ruotato di 90°
    ...
    L_b = 0.1
    offset_b = 0.1
    pos_b1 = pos_cap2 + np.array([d/2 + offset_b, 0, 0])
    bending1 = dm.Dipole_mag(B0, R, L_b, pos_b1, n_hat)
    ...
    QF4 = qm.Quadrupole(R, L, pos4, n_hat, g4, roll1, polarity=+1)  # F in un piano, D nell'altro
    QD4 = qm.Quadrupole(R, L, pos4 + np.array([L + S, 0, 0]), n_hat, g4, roll2, polarity=+1)  # ruotato di 90°
    ...
    multipoles = [QF1, QD1, QF2, QD2, bending1]#, QF3, QD3, QF4, QD4]
    ...
    H, extent = vsl.plane_crossings_density(
        runner.POS, P0, n_hat,
        nbins=(15, 15),
        half_extent=(1e-3, 1e-3),  # finestra visualizzata
